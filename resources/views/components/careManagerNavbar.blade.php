        ...
        <div class="collapse navbar-collapse justify-content-end bg-light" id="navbarNav">
            <ul class="navbar-nav">
                <li class="nav-item dropdown me-2">
                    <a class="nav-link position-relative {{ Request::routeIs('care-manager.messaging.*') ? 'active' : '' }}" href="#" id="messagesDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end message-dropdown p-0" aria-labelledby="messagesDropdown">
                        <li class="dropdown-header">
                            <div class="d-flex justify-content-between align-items-center">
                                <span>Messages</span>
                                <a href="{{ route('care-manager.messaging.index') }}" class="text-decoration-none">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadUnreadMessageCount() {
        fetch('{{ route("care-manager.messaging.unread-count") }}') // Change to care-worker for care worker navbar
    ...
    function loadRecentMessages() {
        fetch('{{ route("care-manager.messaging.recent") }}') // Change to care-worker for care worker navbar
            .then(response => response.json())
            .then(data => {
